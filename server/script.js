...
app.get("/feedback/:userId", async (req, res) => {
  try {
    ...
    const feedback = await Feedback_model.find({ userId }).sort({ createdAt: -1 });
...
app.put("/feedback/:id", async (req, res) => {
  try {
    ...
    const updated = await Feedback_model.findByIdAndUpdate(
      id,
...
app.post("/forum", async (req, res) => {
  try {
    ...
    res.status(201).send({ message: "Message posted and notifications sent", newMsg });
  } catch (error) {
    console.log(error);
    res.status(500).send({ message: "Server error" });
  }
...
app.get("/forum", async (req, res) => {
  try {
    ...
    console.log("GET /forum-message error:", error);
    res.status(500).send({ message: "Server error" });
  }
});
...
app.delete("/forum/:id", async (req, res) => {
  try {
    ...
    const deleted = await ForumMessage.findByIdAndDelete(id);
    if (!deleted) return res.status(404).send({ message: "Message not found" });

   
    await Notification.deleteMany({
      message: new RegExp(deleted.title, "i"),
      type: "forum",
    });

    res.status(200).send({ message: "Forum message & related notifications deleted" });
  } catch (error) {
    console.log("DELETE /forum-message/:id error:", error);
    res.status(500).send({ message: "Server error" });
  }
});
